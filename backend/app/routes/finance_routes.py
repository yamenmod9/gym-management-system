...
def get_expenses():
    ...
    # Calculate totals
    pending_total = sum(e.amount for e in items if e.status == ExpenseStatus.PENDING)
    approved_total = sum(e.amount for e in items if e.status == ExpenseStatus.APPROVED)
